	...
	if (files.length) {
		...
		files.forEach(item => {
			...
			item.onchange = function () {
				...
				// объявим флаг (в значении true говорит что мы работаем со множественной вставкой файлов(здесь-изображений)):
				let multiple = false

				// переменная будет родительским контейнером в галерее, т.е. в который добавлены изображения и есть пустые ячейки (для новых картинок)
				let parentContainer

				// переменная для контейнера в галерее, в который изображение не добавлено (пустой)
				let container

				// проверим является ли искомый input[type=file], элементом формы с множественным добавлением файлов (т.е. установлен ли у него атрибут: multiple)
				// если атрибут (поданный на вход: multiple) есть, то метод: hasAttribute() вернёт: true
				if (item.hasAttribute('multiple')) {

					multiple = true

					// получим родительский контейнер (его элементы в котором лежат файлы (изображения) и пустые ячейки) При этом если файл(картинка) в элементе есть, то это ссылка (a), пустые - это div
					parentContainer = this.closest('.gallery_container')

					if (!parentContainer) return false;

					// получим у родительского контейнера элементы в которох не лежат файлы (изображения), т.е. пустые и в вёрстке они описаны в div
					...
					if (container.length < this.files.length) {
						...
						// после добавления пустых квадратиков перезапишем св-во: container (т.е. сохраним в него изменения кол-ва пустых квадратиков)
						container = parentContainer.querySelectorAll('.empty_container')
					}
				}
				...
				for (let i in this.files) {
					...
					// проверим является ли св-во: i, свойством данного элемента (объекта): files
