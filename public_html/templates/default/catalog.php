							...
							<div class="catalog-aside__wrap">
								<div class="catalog-aside-block">
									<div class="catalog-aside-block__top">
										...
										<button class="catalog-filter-wrap__remove button1" onclick="location.href = location.pathname">Очистить все</button>
									</div>
									<div class="catalog-aside-block__content catalog-aside-block__drop">
										<div class="catalog-aside-block__drop-close">
											<svg viewBox="0 0 27.33 27.01" width="50%" height="50%">
											...
											<?php if (!empty($catalogPrices)) : ?>
												...
												<div class="catalog-filter-item catalog-filter-item_open">
													...
													<div class="catalog-filter-item__content">
														<div class="catalog-range-slider" style="margin-bottom: 25px;">
															<div class="catalog-filter-range__inputs">
																<div class="catalog-filter-range__limit">
																	<div class="catalog-filter-range__text">От</div>
																	<input name="min_price" type="text" value="<?= $catalogPrices['min_price'] ?>" class="catalog-filter-range__input js-rangeSliderMinimal">
																</div>
																<div class="catalog-filter-range__limit">
					...
					<section class="catalog-section catalog-section__four">
						<div class="catalog-section-top">
							...
							<div class="catalog-section-top-items">
								...
								<?php if (!empty($order)) : ?>
									...
									<div class="catalog-section-top-items__title catalog-section-top-items__unit sort">
										Сортировать по:
									</div>
									...
									<?php foreach ($order as $name => $item) : ?>
										...
										<a href="<?= $this->alias('catalog/' . ($this->parameters['alias'] ?? ''), array_merge($GET, ['order' => $item])) ?>" class="catalog-section-top-items__unit catalog-section-top-items__toggle button2 <?= preg_match('/_desc$/', $item) ? 'order-desc' : '' ?>">
						...
						<div class="s-content" style="margin-top: 35px">
							...
							<div class="content-block">
								<p class="text-attention"><?= ($data['name'] !== 'Каталог') ? $data['name'] : $this->set['keywords'] ?> в Донецке, Макеевке, ДНР есть на нашем складе по адресу: <?= $this->set['address'] ?>. Сделать заказ можно на этом сайте. Цены доступные. В ассортименте есть <?= ($data['name'] !== 'Каталог') ? $data['name'] : $this->set['keywords'] ?> от известных поставщиков. Есть возможность доставки по Донецку, Макеевке, ДНР и прилегающей территории</p>
